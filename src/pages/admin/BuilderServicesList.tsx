...
export default function BuilderServicesList() {
  const [items, setItems] = useState<BuilderService[]>([])
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Gerencie os serviços exibidos na página de Construtoras.
          </p>
        </div>
        <Button asChild className="shrink-0 shadow-sm">
          <Link to="/admin/builder-services/new">
              ...
              {loading ? (
                ...
                items.map((item) => (
                  <TableRow key={item.id} className="group hover:bg-slate-50/50 transition-colors">
                    ...
                    <TableCell className="py-4 text-slate-600 text-sm font-mono">
                      {item.icon || '-'}
                    </TableCell>
                    ...
                    <TableCell className="text-right pr-4">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity focus-within:opacity-100">
                        ...
                        >
                          <Link to={`/admin/builder-services/${item.id}`}>
